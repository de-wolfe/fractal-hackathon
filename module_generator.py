...
class ModuleGenerator:
    ...
    def generate_assessment(self, subject, learning_style, previous_module):
        """Generate initial assessment content using the selected LLM."""
        messages = [
            {
                "role": "system",
                "content": "You are a helpful tutor who creates assessments. Return only Streamlit code for a short assessment (1-2 questions).",
            },
            {
                "role": "system",
                "content": f"Module overview: {previous_module}",
            },
            {
                "role": "user",
                "content": f"Generate a concise assessment to test understanding of {subject}. Return only Streamlit code.",
            },
        ]
        content = self.generate_completion(messages, default_model="gpt-4o")
        return clean_streamlit_code(content)
